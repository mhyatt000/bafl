...
def make_oxe_dataset_kwargs(
    ...
    if dataset_kwargs["action_encoding"] not in [ActionEncoding.EEF_POS, ActionEncoding.EEF_R6]:
        e =  ValueError(f"Cannot load `{dataset_name}`; only EEF_POS & EEF_R6 actions supported!")
        overwatch.warning(f"`{dataset_name}` - Error: {e}")

    print(f"Loading `{dataset_name}` from `{data_root_dir}`")
    ...
    elif dataset_kwargs["action_encoding"] is ActionEncoding.HUMAN:
        dataset_kwargs["absolute_action_mask"] = [True] * 111 # human meshes are absolute right now
